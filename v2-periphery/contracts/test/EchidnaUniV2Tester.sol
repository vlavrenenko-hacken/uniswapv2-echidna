...
contract EchidnaUniV2Tester is Setup {
    ...
    function testProvideLiquidityInvariants(uint amount1, uint amount2) public {
        ...
        uint pairBalanceBefore = pair.balanceOf(address(user));
        ...
        (bool success, ) = user.proxy(address(router),abi.encodeWithSelector(router.addLiquidity.selector, address(testToken1), address(testToken2), amount1, amount2, 0, 0, address(user), ~uint(0)));
        
        //POSTCONDITIONS
       
        if (success) {
            (uint reserve1After, uint reserve2After) = UniswapV2Library.getReserves(address(factory), address(testToken1), address(testToken2));
            uint pairBalanceAfter = pair.balanceOf(address(user));
    ...
    function testSwapTokensInvariants(uint swapAmountIn) public {
       //PRECONDITIONS:
       ...
       if(!completed) {
        ...
        uint kBefore = reserve1Before * reserve2Before; 
        //CALL: 
        (bool success, ) = user.proxy(address(router), abi.encodeWithSelector(router.swapExactTokensForTokens.selector, swapAmountIn,0,path,address(user),~uint(0)));
        //POSTCONDITIONS:
    ...
    function testRemoveLiquidityInvariants(uint lpAmount) public {
        ...
        uint kBefore = reserve1Before * reserve2Before;
       (bool success1,) = user.proxy(address(pair),abi.encodeWithSelector(pair.approve.selector,address(router), ~uint(0)));
        require(success1);
        //CALL:
        
        (bool success, ) = user.proxy(address(router),abi.encodeWithSelector(router.removeLiquidity.selector, address(testToken1), address(testToken2),lpAmount, 0, 0, address(user), ~uint(0)));
